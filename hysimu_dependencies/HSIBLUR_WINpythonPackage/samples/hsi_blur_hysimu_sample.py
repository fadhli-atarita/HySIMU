#!/usr/bin/env python
"""
Sample script that uses the HSIBLUR_WIN package created using
MATLAB Compiler SDK.
...
try:
    my_HSIBLUR_WIN = HSIBLUR_WIN.initialize()
except Exception as e:
    print('Error initializing HSIBLUR_WIN package\\n:{}'.format(e))
    exit(1)
...
try:
    ...
    PSF_tot_3dOut, conv_kerOut = my_HSIBLUR_WIN.HSI_BLUR(flight_line_headingIn, FOV_degIn, pix_totIn, altIn, speedIn, itIn, cross_track_sumIn, FWHM_optIn, pix_size_IMGIn, nargout=2)
    print(PSF_tot_3dOut, conv_kerOut, sep='\\n')
except Exception as e:
    print('Error occurred during program execution\\n:{}'.format(e))

my_HSIBLUR_WIN.terminate()