...
def generate_fields(
    ...
    # Else, generate a fractal field and discretize
        ...
        k = np.sqrt(kx**2 + ky**2)
        k[0, 0] = 1  # division by zero prevention
...
def add_spatial_texture(
    ...
    """
    Add texture into map regions using randomized gaussian field. The field
    will be populated by the synthesized spectra of each input endmember
    from the output of the hysimu_spectral_texture function as "spatial
    texture"
    ...
    Parameters:
        ...
        - num_subregions (int): the number of subregions that corresponds to
                                    ...
                                    recommended value is a quarter of the
                                    minimum dimension size
        - region_map (array): the region map generated by the above function
        - spatial_nugget (int, list): a list of spatial nuggets that
                                        determines the spatial distribution
                                        of the texture. recommended value
                                        > 0.01. default value is 0.05
    ...
    # Initialize gstools random seed
    seed_lyr = gs.random.MasterRNG(None)
    ...
    for region in range(num_regions):
        ...
        # Generate random field within each spectral zone with gaussian
        # distribution with gstools package
        # Determine length scale of the field based on:
        len_scale = (1 / num_regions) * min(num_row, num_col)
        # Set grid
        x = np.arange(num_row)
        y = np.arange(num_col)
        # Generate gstools model
        # Spatial nugget determines the spatial distribution of the texture
        model = gs.Gaussian(
            dim=2,
            var=1,
            nugget=spatial_nugget[region],
            len_scale=len_scale
        )
        srf = gs.SRF(model, seed=seed_lyr())
        region_texture = srf.structured([x, y])
